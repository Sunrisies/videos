...
class ThreadSafeFileMerger:
    ...
    def _create_file_list(
        ...
        Args:
            file_list: 文件URL或路径列表（已按M3U8顺序排列）
            workspace: 隔离工作空间
            temp_dir: 临时目录路径
        ...
        try:
            with open(workspace.file_list_path, 'w', encoding='utf-8') as f:
                for file_path in file_list:
                    ...
                    if not os.path.exists(local_path):
                        self.logger.error(
                            f"[{workspace.task_id}] 文件不存在: {local_path}")
                        return False
            ...
            self.logger.info(
                f"[{workspace.task_id}] 文件列表创建成功: {len(file_list)} 个文件（保持M3U8原始顺序）")
            return True
    ...
    def _run_ffmpeg_merge(
        ...
        try:
            ...
            result = subprocess.run(
                ...
                timeout=self.config.merge_timeout if hasattr(
                    self.config, 'merge_timeout') else 600,
                check=False
            )
            ...
            if result.returncode != 0:
                ...
                self.logger.error(
                    f"[{workspace.task_id}] FFmpeg失败: {stderr[:500]}")
                return False
            ...
            self.logger.info(
                f"[{workspace.task_id}] 合并成功: {output_file} ({file_size:,} bytes)")
    ...
    def merge_files(
        ...
        with IsolatedMergeWorkspace(self.base_temp_dir, task_name) as workspace:
            ...
            # 1. 创建文件列表
            if not self._create_file_list(file_list, workspace, temp_dir):
                return False
    ...
    def merge_files_binary(
        self,
        file_list: List[str],  # 注意：参数名改为file_list，表示保持原始顺序
        output_file: str,
        task_name: str = "unknown",
        temp_dir: str = None
    ) -> bool:
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                success = self.merger.merge_files(
                    file_list, output_file, task_name)
                if success:
                    return True
            except Exception as e:
...
class CompatibleFileMerger:
    ...
    def merge_files(self, file_list: List[str], output_file: str, temp_dir: str) -> bool:
        ...
        # 使用新的线程安全合并器，传递临时目录
        success = self._merger.merge_files(
            local_files, output_file, task_name, temp_dir)
    ...
    def merge_files_binary(self, file_list: List[str], output_file: str, temp_dir: str) -> bool:
        """二进制合并（线程安全版本，保持M3U8原始顺序）"""

        task_name = Path(output_file).stem

        # 提取本地文件路径（保持原始顺序）
        local_files = []
        for url in file_list:  # 保持M3U8中的原始顺序，不排序
            filename = self._extract_filename(url)
            filepath = os.path.join(temp_dir, filename)
            local_files.append(filepath)

        # 传递临时目录给底层合并器（保持顺序）
        success = self._merger.merge_files_binary(
            local_files, output_file, task_name, temp_dir)
