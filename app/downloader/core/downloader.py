...
class DownloadManager:
    ...
    def download_batch(self, urls: List[str], save_dir: str, progress_desc: str = "下载进度") -> Dict[str, bool]:
        ...
        try:
                ...
                for url in urls:
                    ...
                    def create_download_task(url, filename):
                        def task():
                            ...
                            try:
                                success = self.download_file(url, save_dir, filename, None)
                    ...
                    future = executor.submit(create_download_task(url, filename))
                    futures[future] = url
            ...
            if self.config.show_progress:
                print()  # 换行
                print(f"下载完成: {completed_count}/{total_count} 成功, {failed_count} 失败")
...
class M3U8Downloader:
    ...
    def download(self, output_file: str = None) -> bool:
        ...
        try:
            ...
            # 解析M3U8
            print("\n解析M3U8文件...")
            ts_files, parse_info = self.parser.parse_m3u8(self.url, self.config.headers)
            ...
            if self.manager.logger:
                self.manager.logger.info(f"分辨率: {parse_info.get('resolution', 'N/A')}")
                self.manager.logger.info(f"带宽: {parse_info.get('bandwidth', 'N/A')}")
            
            # 检查已下载的文件
            downloaded = self.manager.get_downloaded_files(self.config.temp_dir, ts_files)
            if downloaded:
            ...
            if not self.manager.stop_flag:
                print("\n开始合并文件...")
                success = self.manager.merge_files(ts_files, output_file, self.config.temp_dir)
