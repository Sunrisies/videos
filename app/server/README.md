...
- 🔗 **Downloader 集成**: 接收来自 Python Downloader 的视频文件
...
**注意**: `public` 目录是 Downloader 的默认输出目录，下载的视频会自动保存到这里。
...
本服务器设计为与 Python Downloader（位于 `app/downloader`）无缝集成。

### 集成工作流程

1. **Downloader 下载视频**：使用 Python 下载器下载 M3U8/MP4 视频
2. **自动保存到 public**：Downloader 将合成的视频文件保存到 `server/public` 目录
3. **Server 自动发现**：服务器自动扫描 public 目录并通过 API 提供访问
4. **Web 前端播放**：前端通过 API 获取视频列表并播放

### Downloader 配置示例

在运行 Downloader 时，确保输出目录指向 Server 的 public 目录：
